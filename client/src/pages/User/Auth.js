...
import LogImage from '../../assets/auth/log.svg';
...
import { login } from '../../helpers/AuthHelpers/LoginHelper';
import { createNewUser } from '../../helpers/AuthHelpers/SignUpHelper';
...
const Auth = () => {
	...
	const { token, setToken, setCurrentUser, setIsLoggedIn } = useAuthContext();
	...
	const signInSubmitHandler = async (event) => {
		...
		try {
			let result = await login(signInEmail, signInPassword);
			...
			if (result.status !== 200) {
				...
				if (result.data.data.login === null) {
					...
					let resssult = null;
					resssult =  getUserData(token, setCurrentUser, setIsLoggedIn, setStatusInfo);
					if(result.phoneNumber !== "" && result.phoneNumber !== null){
	...
	const signUpSubmitHandler = async (event) => {
		...
		try {
			let result = await createNewUser(fullName, email, password);
			...
			<div className="wrapper">
				<div className="forms-container">
					<div className="signin-signup">
						<form onSubmit={signInSubmitHandler} action="#" className="sign-in-form">
						...
						<form onSubmit={signUpSubmitHandler} action="#" className="sign-up-form">
